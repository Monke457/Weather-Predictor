...
def process_data(filepath="../data/london_weather.csv", plot=False):
    ...
    core_weather = weather[["max_temp", "min_temp", "precipitation",
                            "snow_depth", "mean_temp", "pressure",
                            "cloud_cover", "sunshine"]].copy()
    core_weather.index = pd.to_datetime(
        core_weather.index, format="%Y%m%d")
    ...
    core_weather["mean_temp"] = core_weather["mean_temp"] \
        .fillna(mean_temps)
...
def plot_comparison(df, df_lp, col, year=2000, month=6):
    ...
    sns.lineplot(data=subset_1, x=subset_1.index,
                 y=col, label=f"{col} before filter")
    sns.lineplot(data=subset_2, x=subset_2.index,
                 y=col, label=f"{col} after filter")
    plt.xlabel("Date")
    plt.ylabel("Celsius")
    ...
    plt.xlabel("Date")
    plt.ylabel("Celsius")
    plt.title(f"Max Temperature {year} / {month} "
              f"Before and After Lowpass Filter")
    plt.show()
