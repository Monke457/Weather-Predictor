...
def engineer_features(filepath="../data/weather_processed.pkl"):
    ...
    # ---------------------------------------
    columns = ["cloud_cover", "sunshine", "max_temp", "mean_temp",
    ...
    weather["month_of_year_avg"] = weather["max_temp"] \
        .groupby(weather.index.month) \
        .transform(lambda x: x.expanding(1).mean())

    # Day of year average
    weather["day_of_year_avg"] = weather["max_temp"] \
        .groupby(weather.index.day_of_year) \
        .transform(lambda x: x.expanding(1).mean())
...
def add_temporal_abstraction(df, columns, function):
    ...
    temporal_abstraction = (df[columns]
                            .rolling(window=30, min_periods=1)
                            .apply(aggregate_value(function)))
    df_temp = pd.concat(
        [df, temporal_abstraction.add_suffix(f"_{function}_30")]
        , axis=1)
